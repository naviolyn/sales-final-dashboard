    ...
    <div className="grid @5xl/main:grid-cols-4 @xl/main:grid-cols-2 grid-cols-1 gap-4">
      {(isLoading ? Array.from({ length: 4 }) : cards).map(
        (c: any, idx: number) => {
          ...
          const isArProductive = c.id === "ar_productive";
            ...
            <Card key={c.id} className="@container/card">
              <CardHeader>
                ...
                <CardTitle className="font-semibold @[250px]/card:text-3xl text-2xl tabular-nums">
                  {isArProductive && c.totalAr != null ? (
                    /* "42 / 80" — produktif lebih besar, total lebih kecil & redup */
                    <span className="flex items-baseline gap-1">
                      {formatValue(c.value, c.unit)}
                      <span className="text-base font-normal text-muted-foreground">
                        / {new Intl.NumberFormat("id-ID").format(c.totalAr)} AR
                      </span>
                    </span>
                  ) : (
                    formatValue(c.value, c.unit)
